...
const TreeNode = ({ label, children, level = 0, onAddChild, onDeleteChild, isSubcategory = false, parentCategory = null }) => {
...
const LifeTree = () => {
  ...
  const handleAddChild = (parentLabel, newChildName) => {
    setData(prevData => {
      const newData = { ...prevData };
      
      // Helper function to recursively find and update the parent
      const updateNode = (node, targetLabel, newChild) => {
        ...
        for (const key in node) {
          if (typeof node[key] === 'object' && key !== 'isCustom') {
            if (updateNode(node[key], targetLabel, newChild)) {
              return true;
            }
          }
        }
  ...
  const handleDeleteChild = (childLabel) => {
    setData(prevData => {
      const newData = { ...prevData };
      
      // Helper function to recursively find and delete the child
      const deleteNode = (node, targetLabel) => {
        ...
        for (const key in node) {
          if (typeof node[key] === 'object' && key !== 'isCustom') {
            if (deleteNode(node[key], targetLabel)) {
              return true;
            }
          }
        }
